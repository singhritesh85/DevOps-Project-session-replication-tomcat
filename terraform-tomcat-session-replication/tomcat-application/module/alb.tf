...
output "alb_subnet_id" {
  value = data.aws_subnets.subnet_alb.ids
}

# Security Group for ALB
resource "aws_security_group" "appserver_alb" {
  name        = "appserver-ALB"
  description = "Security Group for Application Server ALB"
  vpc_id      = var.vpc_id
...
resource "aws_lb" "test-application-loadbalancer" {
  ...
  security_groups    = [aws_security_group.appserver_alb.id]           ###var.security_groups
  subnets            = data.aws_subnets.subnet_alb.ids         ###var.subnets
...
resource "aws_lb_target_group_attachment" "ec2_instance1_attachment_to_tg" {
  target_group_arn = aws_lb_target_group.target_group.arn
  target_id        = aws_instance.appserver[0].id               #var.ec2_instance_id[0]
